  ...
  CalendarExtension.prototype.buildCurrentBusinessFootprints = function(wholeDay) {
    ...
    if (anyCustomBusinessHours) {
      ...
      for (k = 0, len1 = flatResources.length; k < len1; k++) {
        ...
        let r_bh = resource.businessHours;
        if (r_bh && this.opt('selectConstraintType') == 'nonBusinessHours') {
            r_bh = null;
            r_bh = _.filter(resource.businessHours, function (bh) {
                return bh.block_selection == true;
            });
        }
        r_bh = r_bh ? r_bh : this.opt('businessHours');
        plainFootprints = this._buildCurrentBusinessFootprints(wholeDay, r_bh);
...
ResourceViewMixin = {
  ...
  getResourceText: function(resource) {
    var res_text = this.getResourceTextFunc()(resource);
...
ResourceDayTableMixin = {
  ...
  renderHeadResourceCellHtml: function(resource, date, colspan) {
    return '<th class="fc-resource-cell"' + ' data-resource-id="' + resource.id + '"' +
        (date ? ' data-date="' + date.format('YYYY-MM-DD') + '"' : '') +
        (colspan > 1 ? ' colspan="' + colspan + '"' : '') + '>' +
        htmlEscape(this.view.getResourceText(resource)) + '</th>';
  },
