...
function renderParsedFormat(parsedFormat, date1, date2, separator, isRTL) {
	...
	for (
		...
		leftStr += renderedParts1[leftI] ? renderedParts1[leftI] : "";
	}
...
Grid.mixin({
    ...
    handleSegContextMenu: function(seg, ev) {
		var res = this.publiclyTrigger('stateCtrl', { // can return `false` to cancel
			context: seg.el[0],
...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderSlatRowHtml: function() {
		...
		while (slotTime < view.maxTime) {
			...
			time_slot = pad(slotDate.hours(), 2) + ":" + pad(slotDate.minutes(), 2);
			...
			html +=
				'<tr time_slot="' + time_slot + '" data-time="' + slotDate.format('HH:mm:ss') + '"' +
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (view.isMultiDayRange(componentFootprint.unzonedRange)) {
			...
			if (seg.isStart || seg.isEnd) {
			    ...
			    let s_date = zonedStart.clone().time();
				slot_start = pad(zonedStart.hours(), 2) + ":" + pad(s_date.minutes(), 2);
			}
		}
		else {
		    ...
		    let s_date = seg.footprint.eventInstance.dateProfile.start.clone();
            slot_start = pad(s_date.hours(), 2) + ":" + pad(s_date.time().minutes(), 2);
		}
...
Calendar.prototype.isFootprintAllowed = function(
	...
	if (constraintVal != null) {
        ...
        var foot_in_constraints = this.isFootprintWithinConstraints(componentFootprint, constraintFootprints);
        if (this.opt('selectConstraintType') == "nonBusinessHours" && foot_in_constraints) {
            return false;
        }
        else if (this.opt('selectConstraintType') != "nonBusinessHours" && !foot_in_constraints) {
			return false;
		}
	}
