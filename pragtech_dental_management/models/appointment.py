...
class MedicalAppointment(models.Model):
    ...
    patient_state = fields.Selection([('walkin', 'Walk In'), ('withapt', 'Come with Appointment'),('withdr', 'Come with Doctor')], 'Patients status',
    ...
    sex = fields.Selection([('m', 'Male'), ('f', 'Female'),('i', 'Intermediate') ], 'Gender', track_visibility='onchange')
    ...
    def done(self):
        ...
        if treatment_ids:
            for each in treatment_ids:
                ...
                if each.description.categ_id and each.description.categ_id.property_account_income_categ_id:
                    product_dict['account_id'] = each.description.categ_id.property_account_income_categ_id.id
    ...
    def write(self, vals):
        ...
        if 'patient' in list(vals.keys()) and vals.get('patient',False):
    ...
    @api.multi
    def get_patient_values(self,values_patient):
        return values_patient
    ...
    def checkin(self):
        ...
        if not self.patient:
            ...
            patient_id = self.env['medical.patient'].create(self.get_patient_values(values_patient))
